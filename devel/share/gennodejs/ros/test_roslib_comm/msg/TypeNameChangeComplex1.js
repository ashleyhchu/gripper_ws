...
class TypeNameChangeComplex1 {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.a.forEach((val) => {
      bufferOffset = SameSubMsg1.serialize(val, buffer, bufferOffset);
    });
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new TypeNameChangeComplex1(null);
    // Deserialize message field [a]
    // Deserialize array length for message field [a]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.a = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.a[i] = SameSubMsg1.deserialize(buffer, bufferOffset)
    }
    // Deserialize message field [b]
    len = 10;
    data.b = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.b[i] = SameSubMsg2.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.a.forEach((val) => {
      length += SameSubMsg1.getMessageSize(val);
    });
  ...
  static Resolve(msg) {
    ...
    const resolved = new TypeNameChangeComplex1(null);
    if (msg.a !== undefined) {
      resolved.a = new Array(msg.a.length);
      for (let i = 0; i < resolved.a.length; ++i) {
        resolved.a[i] = SameSubMsg1.Resolve(msg.a[i]);
      }
    }
    else {
      resolved.a = []
    ...
    if (msg.b !== undefined) {
      ...
      for (let i = 0; i < resolved.b.length; ++i) {
        if (msg.b.length > i) {
          resolved.b[i] = SameSubMsg2.Resolve(msg.b[i]);
        }
        else {
          resolved.b[i] = new SameSubMsg2();
        }
      }
    }
    else {
      resolved.b = new Array(10).fill(new SameSubMsg2())
    }
