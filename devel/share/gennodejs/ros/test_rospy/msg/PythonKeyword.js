// Auto-generated. Do not edit!

// (in-package test_rospy.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class PythonKeyword {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.yield = null;
    }
    else {
      if (initObj.hasOwnProperty('yield')) {
        this.yield = initObj.yield
      }
      else {
        this.yield = 0;
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type PythonKeyword
    // Serialize message field [yield]
    bufferOffset = _serializer.int32(obj.yield, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type PythonKeyword
    let len;
    let data = new PythonKeyword(null);
    // Deserialize message field [yield]
    data.yield = _deserializer.int32(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    return 4;
  }

  static datatype() {
    // Returns string type for a message object
    return 'test_rospy/PythonKeyword';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '1330d6bbfad8e75334346fec949d5133';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    int32 yield
    
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new PythonKeyword(null);
    if (msg.yield !== undefined) {
      resolved.yield = msg.yield;
    }
    else {
      resolved.yield = 0
    }

    return resolved;
    }
};

module.exports = PythonKeyword;
