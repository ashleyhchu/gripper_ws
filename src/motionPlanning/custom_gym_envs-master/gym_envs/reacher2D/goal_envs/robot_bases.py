...
class XmlBasedRobot:
	...
	def __init__(self, robot_name, action_dim, obs_dim, self_collision):
		...
		# self.observation_space = gym.spaces.Box(-high, high)  # commented by Pierre to make a goalEnv

		goal_dim = 2    # the goal vector is in a 2D space
		high_goal = np.inf * np.ones([goal_dim])
		self.observation_space = gym.spaces.Dict({
			'observation': gym.spaces.Box(-high_obs, high_obs),
			'achieved_goal': gym.spaces.Box(-high_goal, high_goal),
			'desired_goal': gym.spaces.Box(-high_goal, high_goal)
			})
...
class MJCFBasedRobot(XmlBasedRobot):
	...
	def reset(self, bullet_client):
		...
		# ADDED BY PIERRE TO HAVE A DICT OBSERVATIONS
		info = self.extra_info()
		obs = {'observation': s, 'achieved_goal': info['fingertip_coord'], 'desired_goal': info['target_coord']}
